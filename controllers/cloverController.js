        ...
        .then((premiumClv) => {
          premiumClv.forEach((item) => {
            ...
            const lockDate = new Date(
              datePurchased.setMonth(datePurchased.getMonth() + 3)
            );
...
exports.getClvForMarketPlace = function (id) {
  return new Promise((resolve, reject) => {
    const dateNow = new Date();
    const buyDate = new Date(dateNow.setMonth(dateNow.getMonth() + 3));
          ...
          sellOrderController
            ...
            .then((sellCap) => {
              Clover.find({
                ...
                $and: [{'dateCreated' :{ $lt: buyDate }},{'owner': id},{'_id' : {$nin: clvInSellOrders }}, {'_id': {$nin: sellCap}}]
                ...
                .then((result) => {
                  ...
                  result.forEach((item) => {
                    const clvItem = {
                      ...
                      availableToSell: item.initialAmount / 10,
                      availableToSellUs: (item.initialAmount / 10) * price,
                      amount: item.amount,
